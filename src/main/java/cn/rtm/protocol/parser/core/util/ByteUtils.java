 ...
 */
package cn.rtm.protocol.parser.core.util;
...
public final class ByteUtils {
     ...
     * @return 返回包装后的协议包
     */
    public static ByteBuffer wrapBE(byte[] payload) {
    ...
    /**
     *  返回读取完数据包长度的数据包内容
     * @param data 协议包内容,该协议包前四个字节代表数据包真实长度
     * @return 返回包装后的协议包
     */
    public static ByteBuffer wrapWithNoLength(byte[] data) {
    ...
    public static String getZeroUuid(ByteBuffer buffer) {
        if (buffer == null || buffer.remaining() <= 0) {
            return "";
        }
     ...
     * @return  返回无符号变长字符串
     */
    public static String getUnsignedVarIntString(ByteBuffer buffer) {
        return getString(buffer, ByteUtils.readUnsignedVarInt(buffer) - 1);
    }
    ...
    public static byte[] readArray(ByteBuffer buf, int size) {
    ...
    public static ByteBuffer getRemainingByteBuffer(ByteBuffer buffer) {
        return wrapBE(getRemainingBytes(buffer));
    }
